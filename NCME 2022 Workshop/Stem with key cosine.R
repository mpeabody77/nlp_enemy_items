...
#For any component of the stem, we'll want to clean out the punctuation,
#numbers, white space, and pesky special characters
stem<-removePunctuation(items$Stem)
stem<-tolower(stem)
...
#lemmatize words
 ...
 sp3<-str_split(sp3," ")
 idword<-rbind(idword,cbind(rep(paste("stem_",items$ItemID[i],sep=""),length(sp2[[1]])),sp[[1]],sp2[[1]],sp3[[1]]))
}


#repeat for key
key<-removePunctuation(items$Key)
key<-tolower(key)
...
for(i in 1:length(key)){
 ...
 sp<-str_split(sp," ")
#stem words - I believe the default here is specific to English
 sp2<-stemDocument(key[i])
 sp2<-str_split(sp2," ")
#lemmatize words
 sp3<-lemmatize_strings(key[i])
 sp3<-str_split(sp3," ")
 idword<-rbind(idword,cbind(rep(paste("key_",items$ItemID[i],sep=""),length(sp2[[1]])),sp[[1]],sp2[[1]],sp3[[1]]))
}
...
ptype<-matrix(substr(stempairs[,1],1,3),ncol=1)
stempairs<-subset(stempairs,ptype=="key")

write.table(stempairs,"key stem pairs.csv",sep=",",append=FALSE,row.names=FALSE,col.names=TRUE)
